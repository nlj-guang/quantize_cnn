...
from vgg import vgg, quan_w_vgg, qa_vgg, vgg_guided
...
parser.add_argument('--weight-quantized', default='vgg/vgg11_qw/model_best.pth.tar', type=str,
                    help="quantize_tanh weight model path")
parser.add_argument('--save-dir', default='vgg/vgg11', type=str,
                    help='directory to save trained model',required= True)
parser.add_argument('--mode', default=0, type=int, help='model quantized mode')
...
parser.add_argument('--balance', default=100, type=float,
                    help='balancing parameter (default: 100)')
...
# 不过来低精度模型的学习率可以调高一点
parser.add_argument('--lr', default=0.01, type=float,  # 论文中初始学习率 0.001, 每 10 epoch 除以 10
                    help='initial learning rate')
parser.add_argument('--rate', default=1, type=int,
                    help='guide training method, full_lr = low_lr * rate')

parser.add_argument('--lr-step', default=15, type=int, help='learning rate step scheduler')
...
def main():
        ...
        if os.path.isfile(args.weight_quantized):
            ...
            for k, v in quantized_model['state_dict'].items():
                if k in model.state_dict():
                    if k.find("conv") != -1 or k.find("clas") != -1:
            ...
            model.load_state_dict(model_dict)
            """
            init_model = torch.load(args.weight_quantized)
    ...
    # lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [74,99])
