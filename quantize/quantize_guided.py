...
from quantize.quantize_activation import quantize_activations_gemm
...
def guided(args):
    ...
    if args.resume:
        full_prec_resume = os.path.join(args.save_dir, "full_prec-checkpoint.pth.tar")
        low_prec_resume = os.path.join(args.save_dir, "low_prec-checkpoint.pth.tar")
        if os.path.isfile(full_prec_resume) and os.path.isfile(low_prec_resume):
            print("=> loading low_prec_checkpoint from '{}' and '{}'".format(full_prec_resume,
                                                                             low_prec_resume))
            full_prec_checkpoint = torch.load(full_prec_resume)
            low_prec_checkpoint = torch.load(low_prec_resume)

            args.start_epoch = low_prec_checkpoint['epoch']
            # 模型的最好精度
            best_low_prec1 = low_prec_checkpoint['best_prec1']
            best_full_prec1 = full_prec_checkpoint['best_prec1']
            ...
            warnings.warn("=> no checkpoint found at directory'{}'".format(args.save_dir))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint({
            ...
            'optimizer': full_prec_optimizer.state_dict(),
