...
class QALinear(nn.Linear):

    def __init__(self, in_features, out_features, bias=True):
        super(QALinear, self).__init__(in_features, out_features, bias)
    ...
    def forward(self, input):
        ...
        if self.bias is not None:
            qbias = quantize_bias_gemm(self.bias)
            # qbias = quantize_weights_bias_tanh(self.bias)
        else:
            qbias = None
...
class Scalar(nn.Module):
    ...
    def __init__(self, s):
        ...
        # 根据训练经验, 设为 2.5
        self.scalar = nn.Parameter(torch.tensor([s], requires_grad=True, dtype=torch.float))
